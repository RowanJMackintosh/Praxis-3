...
class L76X(object):
    FORCE_PIN  = digitalio.DigitalInOut(board.D6)
    STANDBY_PIN = digitalio.DigitalInOut(board.D7)
    ...
    def __init__(self,uartx=_uart0,_baudrate = 9600 ):
        if uartx==self._uart1:
            self.ser = busio.UART(baudrate=_baudrate, tx=board.D4,rx=board.D5)
        else:
            self.ser = busio.UART(baudrate=_baudrate, tx=board.D0,rx=board.D1)

        self.StandBy = self.STANDBY_PIN
        self.StandBy.direction = digitalio.Direction.OUTPUT

        self.Force = self.FORCE_PIN
